...
from gsis_master_intelligence_adapter import GSISMasterIntelligenceAdapter
...
class GSISMasterOrchestrator:
    ...
    def execute_method(self, engine, methods, argument):
        for method in methods:
            if hasattr(engine, method):
                return getattr(engine, method)(argument)
    ...
    def run_pipeline(self, signal, context):
        ...
        # The master intelligence adapter consumes a single signal/context
        # object. Merge the runtime context into a copy of the signal so
        # structure, zone, liquidity, candlestick and economic data are not
        # silently discarded at the orchestration boundary.
        intelligence_input = dict(signal)
        intelligence_input.update(context or {})

        intelligence = self.intelligence.analyze(intelligence_input)

        # GSISMasterIntelligenceAdapter exposes the synthesized decision under
        # "fusion". The previous lookup used "decision", which does not exist
        # in the adapter result and reduced every downstream confidence value
        # to zero.
        fusion = intelligence.get("fusion", {})
        ...
        confidence = fusion.get(
            "confidence",
            intelligence.get("final_confidence", 0)
        )
        ...
        trade = self.lifecycle.open_trade(
            signal["symbol"],
            fusion.get("decision_direction", signal["direction"]),
            signal["entry"],
            signal["stop_loss"],
            signal["tp1"],
            confidence,
        ...
        execution = self.execute_method(
            self.execution,
            ["authorize_execution", "authorize_trade"],
            trade
        )
        ...
        # Never submit an execution payload that did not pass execution
        # authorization. This protects the broker boundary from a method
        # fallback or validation failure.
        if execution.get("status") != "EXECUTION AUTHORIZED":
            return {
        ...
        audit = self.audit.record(
            trade["trade_id"],
            governor.get("decision", "UNKNOWN"),
            execution.get("status", "UNKNOWN"),
            broker.get("broker_status", broker.get("status", "UNKNOWN")),
            "VALIDATED"
        )
...
if __name__ == "__main__":
    ...
    context = {
        ...
        "liquidity": {
            "liquidity_sweeps": ["SELL_SIDE_SWEEP"]
        },
        "candlestick": {
    ...
    engine.run_pipeline(signal, context)
